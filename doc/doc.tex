...
while \system's output accepts (int, any) (in Grift, (Tuple Int Any))
and returns int (in Grift, Int). Rather than a recursive type of Int
paired with a thunk of itself, \system infers Int paired with the dynamic
type. Inspecting each function remaining, you will see that every single (Rec
s (Tuple Int (-> s))) is replaced with (Tuple Int Any) and no other types are
changed. (stream-unfold represents a tuple as a unit-terminated list of pairs,
